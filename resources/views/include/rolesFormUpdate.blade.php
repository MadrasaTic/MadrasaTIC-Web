<form class="row" id="modal_update--form" action="/roles" method="POST">
    @csrf
    <div class="mb-4 col-md-12">
        <div class="input-group">
            <input type="text" class="needs--validation w-100 p-3" placeholder="Nom/Code" name="name" id="name"
                disabled />
    ...
    <div class="mb-4 col-md-12">
        <div class="input-group">
            <textarea class="text--area w-100 p-3" placeholder="Déscription du rôles" style="height: 25vh" name="description"
    ...
    <div class="row m-0 ">
        @foreach ($permissions as $permission)
            <div class="col-md-3 px-0 py-2 text-center">
                <input type="checkbox" class="btn-check m-0 h-100 w-100" id="{{ 'edit_btncheck' . $permission['id'] }}"
                    autocomplete="off" name="permissions[]" value="{{ $permission['id'] }}">
                <label class="btn btn-outline-primary"
                    for="{{ 'edit_btncheck' . $permission['id'] }}">{{ $permission['display_name'] }}</label>
            </div>
        @endforeach
    </div>
